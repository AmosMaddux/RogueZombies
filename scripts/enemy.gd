...
func start_attack():
	...
	for i in range(3):
		...
		tween.tween_property(visuals, "scale", Vector2(1.2, 1.2), 0.1).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
		tween.tween_property(animated_sprite_2d, "self_modulate", Color.RED, 0.1)
		#Snap back to normal size and color
		tween.tween_property(visuals, "scale", Vector2(1.0, 1.0), 0.05).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
